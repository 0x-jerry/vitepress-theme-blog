/**
 * modify based on: https://github.com/vitejs/vite/blob/main/packages/vite/src/node/plugins/asset.ts
 */
...
export const assetCache = new WeakMap<ResolvedConfig, Map<string, string>>()

export const assetHashToFilenameMap = new WeakMap<ResolvedConfig, Map<string, string>>()
// save hashes of the files that has been emitted in build watch
export const emittedHashMap = new WeakMap<ResolvedConfig, Set<string>>()
...
export function checkPublicFile(url: string, { publicDir }: ResolvedConfig): string | undefined {
...
async function fileToBuiltUrl(
  ...
  if (
    config.build.lib ||
    (!file.endsWith('.svg') && content.length < Number(config.build.assetsInlineLimit))
  ) {
